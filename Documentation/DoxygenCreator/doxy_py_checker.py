...
class DoxyPyChecker:
    ...
    @param exclude_patterns : exclude pattern from doxygen configuration use in this checker too
    @param print_checked_files : status if checked files should print
    """

    def __init__(self, path: Optional[str] = None, exclude_patterns: Optional[list[str]] = None, print_checked_files: bool = True):
    ...
    def get_files(self) -> list[str]:
        ...
        for root, dirs, files in os.walk(self.s_path):
            dirs[:] = [d for d in dirs if os.path.basename(d) not in self.exclude_patterns]  # Remove the directories to be excluded from the walk
            for f in files:
                _file_name, file_type = os.path.splitext(f)
                if file_type == ".py":
                    fullpath = os.path.join(root, f)
                    path_obj = Path(fullpath)
                    # Convert uniformly to POSIX format to use / and \ for pattern path
                    if any(Path(path_obj.as_posix()).match(pattern) for pattern in self.exclude_patterns):
                        continue
